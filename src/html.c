...
{
	hoedown_html_renderopt *state = opaque;
...
hoedown_html_toc_renderer_new(int nesting_level)
{
	...
	hoedown_html_renderopt *state;
	hoedown_renderer       *renderer;

	/* Prepare the state pointer */
	state = malloc(sizeof(hoedown_html_renderopt));
	if (!state) return NULL;
	memset(state, 0x0, sizeof(hoedown_html_renderopt));
